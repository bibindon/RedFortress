...
* レバー、感圧板は使用してはいけない。代わりにレバー２、レバー３、感圧板２、感圧板３を使用しなくてはいけない。
  ...
  * レバー2・レバー3・感圧板2・感圧板3 まとめ（箱＋上下する壁のセット）
    ...
    * 共通ルール
      ...
      * **床自体が足場として機能する**（衝突体あり）。レバー2/3・感圧板2/3は`static_platform`の上に置く必要はなく、**足場と足場の間に置いて床でピット帯を橋渡し**する（地面なしステージ等で有効）。床・箱は`PosY`を0.5超（例: 床0.6・箱0.7）にして地面検証の「ピット内配置」チェックを回避し、床の範囲を地面スクリプトの`static_platforms`に登録する。
...
* 破壊可能オブジェクトで道をふさぐとき破壊可能オブジェクトを縦に二つ重ね、さらに奥にも二つ重ねること。
...
* 地面はすべて落とし穴。動く床、動かない床で足場を構築する。
  * 実装パターン（1-3で確立）
    * ピットは外壁に触れられないため、外周に1.5mの地面の帯を残す（`pits=((-14.5, 14.5, -30.5, 30.5))`等）。

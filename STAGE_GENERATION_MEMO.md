...
* レバー、感圧板は使用してはいけない。代わりにレバー２、レバー３、感圧板２、感圧板３を使用しなくてはいけない。
  ...
  * レバー2・レバー3・感圧板2・感圧板3 まとめ（箱＋上下する壁のセット）
    ...
    * 共通ルール
      ...
      * **床自体が足場として機能する**（衝突体あり）。レバー2/3・感圧板2/3は`static_platform`の上に置く必要はなく、**足場と足場の間に置いて床でピット帯を橋渡し**する。床・箱は`PosY`を0.5超（例: 床0.6・箱0.7）にして地面検証の「ピット内配置」チェックを回避し、床の範囲を地面スクリプトの`static_platforms`に登録する。**【「足場間の橋」は地面なしステージ専用】**。地面ありステージでは足場間隔が5m程度のため、箱（6x6）が両隣の足場（4.5x4.5m）と必ず重なる（1-4実績: めり込みが発生し地上ゲートへ修正）。
...
* **地面ありステージ（落とし穴なしテーマ等）では、レバー3/感圧板3の門を「地上の必須ゲート」として配置し、左右を`collision_wall`で塞ぐ**（1-4実績）。ピットが使えないため壁が代替になる。配置例: 床(0,0.05,-22)・箱(0,0.15,-22)・扉=箱中心±3m・レバーTriggerY=箱PosY+0.6=0.75。壁は箱（X[-3,3]）の左右にRotY90で並べ（X[-16,-3]とX[3,16]、高さ3m）、**壁と箱の隙間を0.5m（<プレイヤー直径0.6m）にすると迂回できない**。床（5x10）は地面に敷く（PosY0.05・top0.1）、スタートは床の外の地面に置く。壁の衝突矩形は「壁が足場と重ならない」ことも確認する（BFSは壁を障害物として扱うため、壁が足場にめり込むと移動を塞ぐ）。
...
* 地面はすべて落とし穴。動く床、動かない床で足場を構築する。
  * 実装パターン（1-3で確立）
    ...
    * レバー2/3・感圧板2/3の床・箱は`PosY`0.5超（床0.6・箱0.7）で足場間のピット帯に置き、床の範囲を`static_platforms`に近似登録する（例: 床5x10→half5.0、床5x14→half7.0）。

...
* レバー、感圧板は使用してはいけない。代わりにレバー２、レバー３、感圧板２、感圧板３を使用しなくてはいけない。
  ...
  * レバー3（両開き上昇扉レバー）
    ...
    * レバーは**箱の外側に2つ（各扉の前）と箱の内側中央に1つ**の合計3つを配置し、すべて同じ`TargetID`を共有する。外側の2つは減らさない。箱外レバーは通路を塞がないよう、扉の中心線から扉幅方向へ1.5mずらす。
    * 箱内レバーは箱の中心座標に置き、`TriggerY=箱のPosY+0.5`（内蔵床の上面）とする。箱内で扉を閉じても、このレバーから再び開けられるようにする。
  * レバー2・レバー3・感圧板2・感圧板3 まとめ（箱＋上下する壁のセット）
    ...
    * 共通ルール
      ...
      * 箱外レバーの`TriggerY`は**設置床の上面**（箱下の床なら箱の`PosY`と同じ）にし、扉の中心線から扉幅方向へ1.5mずらす。箱内レバーの`TriggerY`は**箱のPosY + 0.5**（内蔵床の上面）にする。
...
| 感圧板2 | `lever_box.x` | 1枚 `lever_box_door.x` | 感圧板2つ以上（扉の外側＋箱内） | `lever_box_floor.x`（10x10x1.0） | どの感圧板でも同じ扉が上昇し、箱内から脱出可能（共有WallID・OR） |

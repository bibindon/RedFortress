...
* ステージは全32個あり、ワールドごとに30x60m、60x60m、60x120m、120x120m、120x240mのいずれかの広さを持つ。その中に、木箱や動く床、ダメージ床、ダッシュ床、モンスター、レバーなどを適切に配置したい。ステージは32個あるため配置をAIに任せたい。このテキストは、AIが正しくステージを作成させるための説明テキストである。
...
* 固定地形
  ...
  * 草と木の大量配置（1-1方式）
    ...
    * 現在の1-1では配置行として草1,463個、木110本を登録している。これはインスタンシングによって多数の同一モデルを描画する参考例であり、各ステージでは広さと見える範囲に合わせて数を調整する。
...
* レバー、感圧板は使用してはいけない。代わりにレバー２、レバー３、感圧板２、感圧板３を使用しなくてはいけない。
  ...
  * レバー3（両開き上昇扉レバー）
    ...
    * レバーは**箱の外側に2つ（各扉の前）と箱の内側中央に1つ**の合計3つを配置し、すべて同じ`TargetID`を共有する。外側の2つは減らさない。
    * 箱内レバーは箱の中心座標に置き、`TriggerY=箱のPosY+0.6`とする。箱内で扉を閉じても、このレバーから再び開けられるようにする。
  * レバー2・レバー3・感圧板2・感圧板3 まとめ（箱＋上下する壁のセット）
    ...
    * 共通ルール
      ...
      * **床自体が足場として機能する**（衝突体あり）。レバー2/3・感圧板2/3は`static_platform`の上に置く必要はなく、**足場と足場の間に置いて床でピット帯を橋渡し**する。床の上面と箱の`PosY`を0.5超（例: 1.0m厚床の下面-0.3・上面0.7、箱0.7）にして地面検証の「ピット内配置」チェックを回避し、床の範囲を地面スクリプトの`static_platforms`に登録する。**【「足場間の橋」は地面なしステージ専用】**。地面ありステージでは足場間隔が5m程度のため、箱（6x6）が両隣の足場（4.5x4.5m）と必ず重なる（1-4実績: めり込みが発生し地上ゲートへ修正）。
      * **複数の操作装置で同じ扉を共有できる**（門）。感圧板は同じ`WallID`、レバーは同じ`TargetID`を複数行で指定してよい（`PressurePlateManager`・`AttackTriggerManager`の共有ID重複チェックは削除済み）。
      * **平地ステージでレバー3/感圧板3を「迂回不能な門」にするには、門の左右をピットで塞ぐ**（1-1・1-2実績）。箱（6x6）の両脇 X[-16,7] / X[13,16] の帯（Zは箱と同じ範囲 ±3m）をピットにすると、箱（幅6m）だけが通路になり、レバー/感圧板を操作しないと先へ進めない。門の「奥側」への回り込みも塞ぐこと（ピットをZ方向に奥まで伸ばす）。
* **地面ありステージ（落とし穴なしテーマ等）では、レバー3/感圧板3の門を「地上の必須ゲート」として配置し、左右を`collision_wall`で塞ぐ**（1-4実績）。ピットが使えないため壁が代替になる。配置例: 1.0m厚床(0,-0.85,-22)・箱(0,0.15,-22)・扉=箱中心±3m・レバーTriggerY=箱PosY+0.6=0.75。壁は箱（X[-3,3]）の左右にRotY90で並べ（X[-16,-3]とX[3,16]、高さ3m）、**壁と箱の隙間を0.5m（<プレイヤー直径0.6m）にすると迂回できない**。床（5x10）は上面が地面付近に来るよう敷き（PosY-0.85・top0.15）、スタートは床の外の地面に置く。壁の衝突矩形は「壁が足場と重ならない」ことも確認する（BFSは壁を障害物として扱うため、壁が足場にめり込むと移動を塞ぐ）。

| ギミック | 箱モデル | 扉 | 操作装置 | 床 | 動作 |
|---|---|---|---|---|---|
| レバー2 | `lever_box.x` | 1枚 `lever_box_door.x` | レバー1つ | `lever_box_floor.x`（10x10x1.0） | レバーを攻撃するたびに扉が上昇⇔下降（トグル） |
| レバー3 | `lever_box3.x` | 2枚 `lever_box3_door.x` | レバー3つ（各扉の前に1つずつ＋箱内中央に1つ） | `lever_box3_floor.x`（5x10x1.0） | どのレバーでも両扉が同時に上昇⇔下降（共有トグル）。箱内から再開放可能。**門** |
| 感圧板2 | `lever_box.x` | 1枚 `lever_box_door.x` | 感圧板1つ（扉の前） | `lever_box_floor.x`（10x10x1.0） | 感圧板に乗ると扉が上昇、離れると下降 |
| 感圧板3 | `lever_box3.x` | 2枚 `lever_box3_door.x` | 感圧板2つ（各扉の前） | `lever_box3_floor14.x`（5x14x1.0） | どちらの感圧板でも両扉が同時に開く（共有WallID・OR）。**門** |
...
* 押せる箱
  ...
  * **「押せる箱 + 高台」登りギミック（1-2実績）**: 高台（static_platform_2x2等）の上面を**2.2m以下**にすると、「箱の上（1.2m）→ ジャンプ（1.28m）→ 2.48m」で登れる。箱を高台の縁まで押してから乗る。高台の上に敵+アイテムを置くと「登って戦って取る」報酬型の寄り道になる。
...
* ピットはSTAGES定義の`pits`に`(x0, x1, z0, z1)`で指定する。**ピットの外周接触チェックは2026-08に削除済み** — ピットを外周壁（X=±16 / Z=±32）まで拡張できる。外周まで広げると「外周際を歩いてギミックを迂回する」抜け道を塞げる（1-1: ピットBをX[-16,16]全域化してフェリー必須化 / 1-2: 同様に西端の抜け道を封鎖）。
...
* 名前付きパイプでゲームを操作できる。
* 俯瞰レイアウト図を作成してOKをもらってから作り始めること。
* 破壊可能オブジェクトで道をふさぐとき破壊可能オブジェクトを縦に二つ重ね、さらに奥にも二つ重ねること。横方向については道からはみ出るまで並べてください。
...
* **各ステージに移動床を1つ以上使うこと。**
  * 上記のギミックで以下のどれかが達成できること
    ...
    * ショートカットができる。
    * ダメージ床を避けられる。
* **各ステージにレバー2 or レバー3 を1つ以上使うこと。**
* レバー２の配置は以下のどれかになっていること
  * 高い場所にあり、動く床 or 動かない床 or 木箱 or ダッシュブースターに乗らないと到達できない。
  * 落とし穴の上にあり、動く床 or 動かない床 or 木箱 orダッシュブースターに乗らないと到達できない。
* レバー３の配置は以下のようになっていること
  * レバー３は「門」の役割を持つ
  * 二つの区画の間に配置し、レバー３を通らないと二つの区間を移動できないようにする。
  　例えば、区画Aと区画Bがあるとする。区画Aが落とし穴か壁で囲まれていて、区画Bと区画Aがレバー３で接続されているとする。この場合、区画Aに行くにはレバー３を通るしかない。
* **各ステージに感圧板2 or 感圧板3 or ワープオブジェクト or ダメージ床 or 押せる箱を1つ以上使うこと。**
* 感圧板2の配置は以下のどちらかになっていること
  * 高い場所にあり、動く床 or 動かない床 or 木箱 に乗らないと到達できない。
  * 落とし穴の上にあり、動く床 or 動かない床 or 木箱 に乗らないと到達できない。
* 感圧板３の配置は以下のようになっていること
  * 感圧板３は「門」の役割を持つ
  * 二つの区画の間に配置し、感圧板３を通らないと二つの区間を移動できないようにする。
  　例えば、区画Aと区画Bがあるとする。区画Aが落とし穴か壁で囲まれていて、区画Bと区画Aが感圧板３で接続されているとする。この場合、区画Aに行くには感圧板３を通るしかない。
...
* 地面はすべて落とし穴。動く床、動かない床で足場を構築する。
  * 実装パターン（1-3で確立）
    * ピットは外周壁まで拡張できる（外周接触チェックは2026-08に削除済み）。外周に1.5mの地面の帯を残す実装（`pits=((-14.5, 14.5, -30.5, 30.5))`等）と、外周まで全面ピット化する実装（1-1・1-2実績）のどちらでもよい。全面ピット化すると外周際の徒歩迂回が不可能になる。
    ...
    * 破壊可能オブジェクト（ガレキ）は`XFileListPhysics.csv`に登録しないため検証のBFSは障害物扱いしない。**「ガレキで道を塞いでも検証は通る」**。ゲーム内では衝突体が壊すまで通行を阻む。
...
* 1-4「風の中の登りタワー」
  ...
  * 詳しい説明
    * 落とし穴はナシ。
    * 地面からスタートして上の方にあるゴールを目指して足場を登っていく。
* 1-5「ドカンと大砲通路」
  * 短い説明：ダッシュ板を乗り継いで移動するステージ。
  * 詳しい説明
    * 特になし。
* 1-6「しずむとりでの大砲わたり」
  * 短い説明：ダッシュ床と動かない床がたくさんあるステージ
  * 詳しい説明
    * 地面なし。
* 1-7「ざこだらけ」
  * 短い説明：雑魚敵がたくさんいるステージ
...
* 2-1「鳥だらけ」
  ...
  * 詳しい説明
    * ステージ全体がダメージ床
    * 幅0.5メートルの細い陸地が大半を占めていて、これを歩いてゴールに向かわなければならない。
    * 鳥の攻撃によって陸地から落ちないようにプレイヤーは頑張る必要がある。
* 2-2「上に逃げろ！」
  * 短い説明：下からせり上がる溶岩から逃げ、上にあるゴールを目指すステージ。
  * 詳しい説明
    * 鳥は溶岩ダメージを受けないため、せり上がる溶岩で倒せずクリアできなくなる。鳥を配置しないこと。
...
* 2-3「追ってくるマグマの道」
  ...
  * 詳しい説明
    * 鳥は溶岩ダメージを受けないため、迫る溶岩の後方に残って倒せずクリアできなくなる。鳥を配置しないこと。
    * 60x120mの範囲に全長約188mのS字コースを作り、コース外は落下する地形としてショートカットを防ぐ。
    * 迫る溶岩は5つの直線区間に分け、直前の区間が角へ到達した時点で次の区間を起動する。通過済み区間の溶岩は残す。
    * 道中に敵と破壊可能オブジェクトを配置し、戦うか素早く突破するかを短時間で判断させる。
    * 曲がり角では隣接する溶岩区間を重ね、安全な隙間や追跡の途切れが発生しないようにする。
    * 必須敵を倒しながら進めるよう、溶岩の速度と各区画の広さを調整する。
* 2-4「空飛ぶ足場を乗りつごう」
  * 短い説明：動く床がたくさんあるステージ。
  * 詳しい説明
    * 動く床がたくさんあるステージ。
    * スタート地点はY=1でゴール地点はY=30のあたりにある。
    * 上下に移動する動く床は配置しない。
* 2-5「チクチク床の飛び石」
  * 短い説明：ダメージ床を避けてゴールを目指すステージ。
  * 詳しい説明
    * 特になし
* 2-6「あつあつ飛び石ロード」
  * 短い説明：ダメージ床を避けてゴールを目指すステージ。
  * 詳しい説明
    * 地面はほとんどがダメージ床となっている
    * ダメージ床がない正方形の陸地が4つくらいある
* 2-7「無敵でゴーゴー火の道」
  * 短い説明：スターでダメージ床を駆け抜けるステージ。
  * 詳しい説明
    * スターを取得→走る→無敵時間が終了する前に追加のスターを取得する、これを繰り返してクリアする。
    * 動かない床がいくつかあって休憩できる。動かない床にはアイテムが配置されていたり、モンスターが配置されている。
    * スターは動かない床や岩の上にある。ダメージ床の上に置いてあることもある。
* 2-8「マグマ王のあつあつ城」
  * 短い説明：World 2のボスステージ。
  * 詳しい説明
    * ボスあり版は120x120m内の80x100mの城内闘技場とし、中央へ`boss_golem`を1体だけ配置する。
    * ボスあり版には5枚のダメージ床と岩を少しだけ置き、QTE、移動床、頭蓋骨、押せる箱、感圧板、ダッシュブースター、ワープ、破壊物、アイテム、雑魚敵を配置しない。
    * クリア後版は崩れた左右2本の回廊と中央橋へ地形を切り替え、中央の2か所を大きな落とし穴にする。
    * クリア後版にはWorld 2の雑魚敵12体、破壊可能オブジェクト8個、アイテム4個、QTE木1本、ダメージ床4枚を配置する。
    * クリア後版にもダッシュブースター、レバー、ボタン、ロープは配置しない。

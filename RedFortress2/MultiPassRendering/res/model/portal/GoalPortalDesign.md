...
- 旧モデルの天面はモデルZ=1.5。新モデルはZ=1.0に下げ、登りやすい段差にした。ゲームの柱接触判定は水平距離（半径0.55m）に加え、上下帯（ポータル基準Y +0.7〜+1.6m、定数 `kPortalPillarTouchHeightMin/Max`）を持つ。台座天面は基準+1.016mなので、天面に乗った（または天面へ着地中の）時だけ接触が成立する。

...
const std::wstring kStageSelect1Bgm = L"res\\sound\\stageselect1.wav";
...
void Initialize()
{
    ...
    {
        kMenuMove, kMenuConfirm, kMenuCancel, kMenuOpen, kCraftOpen, kExplanationOpen, kSaveComplete, kPlayerAttack, kSwordSwing, kSlashHit, kAttackHit, kSwordDeflected, kBusterHit,
...
void StopPushableBoxMovement()
{
    ...
    try
    {
        SoundLib::SoundLib::StopEnvironmentSound(g_pushableBoxMovementId);
    }
    catch (const SoundLib::AudioDeviceException&)
    {
        BeginAudioDeviceRecovery();

...
void PauseMenu::Render(const std::wstring& stageName)
{
    ...
    {
        m_stageNameFontId = m_render->SetUpFont(L"BIZ UDGothic", 34, kTextColor);
    }

    if (m_menuItemFontId < 0)
    {
        m_menuItemFontId = m_render->SetUpFont(L"BIZ UDGothic", 30, kTextColor);
    }

    if (m_qualityFontId < 0)
    {
        m_qualityFontId = m_render->SetUpFont(L"BIZ UDGothic", 24, kSubTextColor);
    }

    m_render->DrawTextCenter(m_stageNameFontId,
                             stageName,
                             960,
                             115,
                             560,
    ...
    m_render->DrawTextCenter(m_qualityFontId,
                             L"グラフィック設定",
                             1040,
                             525,
                             180,
    ...
    m_render->DrawTextCenter(m_menuItemFontId,
                             L"拠点に戻る",
                             1180,
                             760,
                             330,

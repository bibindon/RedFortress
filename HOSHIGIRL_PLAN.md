...
### フェーズ4: モデル造形改善（100% 完了 — 2026-07-29）
- [x] **腕を全面的に作り直し**（旧: 細い棒が水平に突き出すだけで爪もほぼ無し）
  - 布袖型: 肩(半径0.10)→肘(0.068)→手首(0.05)のテーパーチューブ、肩は体内に埋めて接続
  - 各手に爪3本（円錐、長さ0.13）
  - L腕: 前方に大きく差し出す構え（参考画像の掲げた手）。R腕: 胸の前で爪を下に向ける構え
  - 腕ボーン6本のレスト位置を新ジオメトリに合わせて移動（アニメは相対変換なので無改修で動作）
  - ウェイト: 肩=Spine混合 → 上腕=UpperArm → 肘混合 → 前腕=LowerArm → 手首混合 → 爪=Hand
- [x] **裾を再構成**（旧: 24頂点の角張ったギア状フレア r0.55〜0.92）
  - 32分割の滑らかな波裾: z = -0.02 + 0.055sin(7θ+0.7) + 0.025sin(3θ+2.1)
  - 裾の内側に折り返しリング＋底面ファンで閉じ、布の厚みを表現
  - 最下端は約-0.10で据え置き（`GetMeshVerticalOffset()=0.25f` の前提を維持）
- [x] 上半身プロファイル（z≥0.3）は旧値を踏襲（顔メッシュとの位置関係を保護）
- [x] Body再生成: 32側面×12リング、微細な縦折りノイズ（±1.6%）で布らしさ追加
- [x] ウェイトは高さベース: Head/Spine/Root をsmoothstepで混合
- [x] UVは全面テクスチャの黒領域(0.04,0.04)に集約（従来通り）
- [x] 再エクスポート＆出力先へ配置済み（idle/deathともレンダリング検証済み）

> **再生成スクリプト**: 作業用スクリプトは `%TEMP%\opencode\` 配下の一時ファイル。
> 再現が必要な場合は本ファイルの手順か git 履歴の blend 差分を参照。
